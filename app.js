...
async function loadBody(){
 try{
  ...
  const r=await fetch(ASSET_URL,{cache:"force-cache",signal:controller.signal});
  clearTimeout(timeout);
  if(!r.ok)throw new Error(`HTTP ${r.status}`);
  let geo=parseBodyOBJ(await r.text());

  // v0.7.1: smooth the known-working v0.5 mesh without changing its loading architecture.
  // The OBJ parser creates duplicate triangle vertices; merge coincident vertices first,
  // then calculate shared normals.
  ...
  function frameWholeBody(){
    ...
    orbit.target.set(center.x, center.y + size.y*.04, center.z);

    const vFov=THREE.MathUtils.degToRad(camera.fov);
    const aspect=Math.max(.42, innerWidth/innerHeight);
    const hFov=2*Math.atan(Math.tan(vFov/2)*aspect);
    const distH=(size.y*.78)/Math.tan(vFov/2);
    const distW=(size.x*.72)/Math.tan(hFov/2);
    const distance=Math.max(distH,distW,4.8);
    ...
    orbit.maxDistance=Math.max(35,distance*7);
    orbit.update();
  }
  frameWholeBody();
  ...
  const reason=err?.name==="AbortError" ? "Timeout beim Laden des Basemeshs" : String(err.message);
  el.querySelector("small").textContent=reason+" · Seite neu laden";

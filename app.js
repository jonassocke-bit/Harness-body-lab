...
const BODY_URL="./base.obj";
const TARGETS={
 waist:{minus:"./waist-decr.target",plus:"./waist-incr.target"},
 hips:{minus:"./hips-decr.target",plus:"./hips-incr.target"}
};
...
orbit.enablePan=true;orbit.screenSpacePanning=true;
orbit.touches.ONE=THREE.TOUCH.ROTATE;
orbit.touches.TWO=THREE.TOUCH.DOLLY_PAN;
orbit.minDistance=.7;orbit.maxDistance=30;
...
const morphs={},values={waist:0,hips:0};
...
function parseBodyOBJ(text){
 ...
 if(verts.length<BODY_VERTEX_COUNT)throw new Error("Basemesh unvollständig");
 ...
 g.setIndex(faces);g.computeVertexNormals();g.normalizeNormals();
 return g;
}
...
async function loadPair(name){
 const [minus,plus]=await Promise.all([
  getText(TARGETS[name].minus),getText(TARGETS[name].plus)
 ]);
...
function updateBody(){
 ...
 for(const name of ["waist","hips"]){
  const v=values[name],pair=morphs[name];
  if(!pair)continue;
 ...
 body.geometry.computeVertexNormals();body.geometry.normalizeNormals();
...
function frameBody(){
 ...
 const dH=(size.y*.72)/Math.tan(vf/2);
 const dW=(size.x*.70)/Math.tan(hf/2);
 const dist=Math.max(dH,dW,4.6);
 camera.position.set(center.x,center.y,center.z+dist);
 orbit.maxDistance=Math.max(30,dist*6);orbit.update();
}
...
async function init(){
 ...
 try{
  const [objText]=await Promise.all([getText(BODY_URL)]);
  const geo=parseBodyOBJ(objText);
  ...
  const mat=new THREE.MeshPhysicalMaterial({
   color:0xd8ccc4,roughness:.64,metalness:0,
   clearcoat:.025,clearcoatRoughness:.9,flatShading:false,side:THREE.DoubleSide
  });
  body=new THREE.Mesh(geo,mat);scene.add(body);frameBody();

  await Promise.all([loadPair("waist"),loadPair("hips")]);
  targetState.textContent="2/2 echte MakeHuman-Morphs lokal aktiv";
  status.classList.add("ok");
  updateBody();
 }catch(err){
  status.classList.remove("loading");status.classList.add("error");
  status.querySelector("strong").textContent="Lokaler Quality Test konnte nicht laden";
  status.querySelector("small").textContent=String(err.message||err);
  targetState.textContent="Ladefehler – Screenshot genügt";
 }
}
for(const name of ["waist","hips"]){
 const input=document.querySelector("#"+name),out=document.querySelector("#"+name+"Out");
...
document.querySelector("#reset").addEventListener("click",()=>{
 ...
 for(const n of ["waist","hips"]){
  document.querySelector("#"+n).value=0;
  document.querySelector("#"+n+"Out").textContent="0";
 }
 updateBody();
});
...
document.querySelectorAll("[data-view]").forEach(btn=>btn.addEventListener("click",()=>{
 ...
 body.geometry.computeBoundingBox();
 const box=body.geometry.boundingBox,size=new THREE.Vector3(),center=new THREE.Vector3();
 box.getSize(size);box.getCenter(center);
 const dist=camera.position.distanceTo(orbit.target);
 if(btn.dataset.view==="front")camera.position.set(center.x,center.y,center.z+dist);
 if(btn.dataset.view==="back")camera.position.set(center.x,center.y,center.z-dist);
 if(btn.dataset.view==="side")camera.position.set(center.x+dist,center.y,center.z);
